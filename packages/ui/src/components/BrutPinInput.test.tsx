...
function ControlledPinInput(
  props: Omit<ComponentProps<typeof BrutPinInput>, "value" | "onChange"> & {
    initial?: string;
  },
) {
  const { initial = "", ...rest } = props;
  const [value, setValue] = useState(initial);
  return <BrutPinInput value={value} onChange={setValue} {...rest} />;
}

describe("BrutPinInput", () => {
  ...
  it("masks digits when secure", () => {
    render(<BrutPinInput value="1234" onChange={() => {}} secure length={4} />);
